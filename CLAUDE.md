...
Mounted in `server.js` via `require('./relationships').mount(app, db, { deepseek, genAI })` (reuses the existing AI clients). Everything lives inside the existing Relationships tab (briefly labelled "People" on 2026-09-17, renamed back on 2026-09-18; the hash is `#relationships`): `#res-rel-people` ("My People") is the first section; the older sections (Love forecast, Compatibility, Profile, Compatibility Check, Soul Animals, Business) are unchanged below it. Client: `app/rel-form.js` (shared birth form) + `app/relationships.js` + `app/relationships.css`, loaded after `script.js`. No new env vars except the optional paywall flag.
